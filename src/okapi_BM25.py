...
class BM25:
    ...
    def read_document(self):
        ...
        for index, row in df.iterrows():
            ...
            # Remove stop words
            tokenized_doc = [word for word in tokens if word.lower() not in self.stop_words]
    ...
    def rank_documents(self, query):
        ...
        bm = realBM25(self.tokens)

        # Preprocess the query
        query_tokenized = word_tokenize(query.lower())

        scores = bm.get_scores(query_tokenized)

        # Sort the RSV
        titles = [key for key in self.data.keys()]
        mapped = list(zip(titles, scores))
